...
def _debug_code_allowed(phone: str) -> bool:
    ...
    Провайдера СМС ещё нет, поэтому на тестовом стенде вход держится на едином
    коде: иначе не войти ни новому гостю, ни старому.

    Настройка `sms_debug_phones` сужает поблажку до перечисленных номеров.
    Пустая — код подходит всем, и это осознанный режим стенда. Как только
    появятся настоящие гости, список заполняется (или подключается СМС), и
    чужие номера закрываются без правки кода.
    """
    if settings.sms_code_debug_value is None or settings.environment == "production":
        return False

    if not settings.sms_debug_phones:
        return True
