...
4. **Безопасные зоны.** `useSafeAreaInsets()` для верха и низа — на устройствах с
   ...
   Модалка — отдельное окно, и edge-to-edge на неё не действует: `<Modal>` без
   `statusBarTranslucent` и `navigationBarTranslucent` оставляет на Android
   просвечивающую полосу под шторкой. Ставим оба свойства всегда.
